...
// On platforms without unix/windows (e.g. wasm32-unknown-unknown),
// fall back to the default 4096 page size for both get() and
// get_granularity(), since there is no OS-provided API to query it.
#[cfg(not(any(unix, windows)))]
#[inline]
fn get_granularity_helper() -> usize {
    4096 // 4k is the default on many systems
